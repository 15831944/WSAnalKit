...
int CLogFile::FormatAdd(int nCurLevel,const char * formatString, ...)
{
	...
	try{
        ...
        */
		char* szThid=(char*)strDes.data();
		sprintf(szThid,"0x%08X",getthreadid());
		int nthLen=strlen(szThid);
		szThid[nthLen]=' ';szThid[nthLen+1]=0;
		vsprintf(szThid+nthLen+1, formatString, argList);
